...
class SitesDb(object):
	...
	def get_by_loc(self, lat, lon, latwidth=3, lonwidth=3):
		...
		tous = self.cursor.execute('''SELECT * FROM SITES WHERE latitude_wgs84>{} AND latitude_wgs84<{} AND longitude_wgs84>{} AND longitude_wgs84<{}'''.format(latmin, latmax, lonmin, lonmax)).fetchall()
		return self.list_to_dict(tous)
